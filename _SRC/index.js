...
const navigation = () => {
    ...
    nav.forEach((el, index) => {
        el.addEventListener('click', (e) => {
            e.preventDefault();
            console.log(index);
            // if (document.documentElement.clientWidth <= 980) page = document.querySelectorAll('.page');
            ...
            page[index % 4].classList.add('active');

            document.body.classList = '';

            animText(page[index % 4]);
            ...
            document.body.classList.add(`style${index % 4 + 1}`)
        });
    });
}
let tmp = 0;
